...
export default function DyePage() {
  ...
  useEffect(() => {
    const columnVisibility = localStorage.getItem(
      'mrt_columnVisibility_DyePage',
    );
    const columnSizing = localStorage.getItem(
      'mrt_columnSizing_DyePage',
    );
  ...
  useEffect(() => {
    if (isFirstRender.current) return;
    localStorage.setItem('mrt_sorting_DyePage', JSON.stringify(sorting));
  }, [sorting]);

  useEffect(() => {
    if (isFirstRender.current) return;
    localStorage.setItem('mrt_columnSizing_DyePage', JSON.stringify(columnSizing));
  }, [columnSizing]);

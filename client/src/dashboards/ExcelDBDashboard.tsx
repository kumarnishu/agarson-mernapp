    ...
    useEffect(() => {
        ...
        if (user && categoryData && categoryData.data) {
            categoryData.data.map((dt) => {
                tmpfeatures.push({ feature: dt.value, display_name: dt.label && dt.label, is_visible: true, url: `ExcelDbReports/${dt.id}` })
            })
        }
                        ...
                        <Grid key={index} item xs={12} md={4} lg={3} sx={{ p: 1 }}>
                            <Link to={feat.url} style={{ textDecoration: 'none' }}>
                                ...
                                >
                                    ...
                                    <br />
                                    {feat.display_name && <Typography title={toTitleCase(feat.display_name)} sx={{ pl: 4.5,fontSize:11 }}>{`${feat.display_name} `}</Typography>}
                                </Paper>
                            </Link>
                        </Grid>
                    )

...
function SalesmanKpiPage() {
  ...
  useEffect(() => {
    const columnVisibility = localStorage.getItem(
      'mrt_columnVisibility_SalesmanKpiPage',
    );
    const columnSizing = localStorage.getItem(
      'mrt_columnSizing_SalesmanKpiPage',
    );
  ...
  useEffect(() => {
    if (isFirstRender.current) return;
    localStorage.setItem('mrt_sorting_SalesmanKpiPage', JSON.stringify(sorting));
  }, [sorting]);

  useEffect(() => {
    if (isFirstRender.current) return;
    localStorage.setItem('mrt_columnSizing_SalesmanKpiPage', JSON.stringify(columnSizing));
  }, [columnSizing]);

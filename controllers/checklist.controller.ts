...
import { getFirstMonday, nextYear, previousYear } from "../utils/datesHelper";
import moment from "moment";
import isMongoId from "validator/lib/isMongoId";
import { GetChecklistDto, CreateOrEditChecklistDto, GetChecklistFromExcelDto } from "../dtos/checklist.dto";
import { IChecklistBox, ChecklistBox } from "../models/checklist-box.model";
import { ChecklistCategory } from "../models/checklist-category.model";
import { ChecklistRemark } from "../models/checklist-remark.model";
import { Checklist, IChecklist } from "../models/checklist.model";
import { IUser, Asset, User } from "../models/user.model";
...
export const GetChecklists = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!Number.isNaN(limit) && !Number.isNaN(page)) {
        if (req.user?.assigned_users && req.user?.assigned_users.length > 0 && id == 'all') {
            ...
            checklists = await Checklist.find({ assigned_users: req.user?._id }).populate('created_by').populate('updated_by').populate('category').populate('lastcheckedbox').populate('last_10_boxes').populate('assigned_users').sort('created_at').skip((page - 1) * limit).limit(limit)
            count = await Checklist.find({ assigned_users: req.user?._id }).countDocuments()

        }

        else {
            checklists = await Checklist.find({ assigned_users: id }).populate('created_by').populate('updated_by').populate('category').populate('assigned_users').populate('lastcheckedbox').populate('last_10_boxes').sort('created_at').skip((page - 1) * limit).limit(limit)
...
export const GetMobileChecklists = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (category !== 'all') {
        ...
        }).populate('lastcheckedbox').populate('last_10_boxes').populate('updated_by').populate('category').populate('assigned_users').sort('created_at')
    }
    else

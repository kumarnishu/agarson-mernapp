...
export default function RefersPage() {
  ...
  useEffect(() => {
    const columnVisibility = localStorage.getItem(
      'mrt_columnVisibility_RefersPage',
    );
    const columnSizing = localStorage.getItem(
      'mrt_columnSizing_RefersPage',
    );
  ...
  useEffect(() => {
    if (isFirstRender.current) return;
    localStorage.setItem('mrt_sorting_RefersPage', JSON.stringify(sorting));
  }, [sorting]);

  useEffect(() => {
    if (isFirstRender.current) return;
    localStorage.setItem('mrt_columnSizing_RefersPage', JSON.stringify(columnSizing));
  }, [columnSizing]);

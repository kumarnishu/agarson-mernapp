...
export default function SalesAttendanceReportPage() {
  ...
  useEffect(() => {
    const columnVisibility = localStorage.getItem(
      'mrt_columnVisibility_SalesAttendanceReportPage',
    );
    const columnSizing = localStorage.getItem(
      'mrt_columnSizing_SalesAttendanceReportPage',
    );
  ...
  useEffect(() => {
    if (isFirstRender.current) return;
    localStorage.setItem('mrt_sorting_SalesAttendanceReportPage', JSON.stringify(sorting));
  }, [sorting]);

  useEffect(() => {
    if (isFirstRender.current) return;
    localStorage.setItem('mrt_columnSizing_SalesAttendanceReportPage', JSON.stringify(columnSizing));
  }, [columnSizing]);

...
function PaymentsPage() {
  ...
  useEffect(() => {
    const columnVisibility = localStorage.getItem(
      'mrt_columnVisibility_PaymentsPage',
    );
    const columnSizing = localStorage.getItem(
      'mrt_columnSizing_PaymentsPage',
    );
  ...
  useEffect(() => {
    if (isFirstRender.current) return;
    localStorage.setItem('mrt_sorting_PaymentsPage', JSON.stringify(sorting));
  }, [sorting]);

  useEffect(() => {
    if (isFirstRender.current) return;
    localStorage.setItem('mrt_columnSizing_PaymentsPage', JSON.stringify(columnSizing));
  }, [columnSizing]);
  console.log(payment)
  return (
    <>

...
export default function LeaveBalancePage() {
  const [article, setArticle] = useState<GetLeaveDto>()
  const [articles, setArticles] = useState<GetLeaveDto[]>([])
  const [hidden, setHidden] = useState(false)
  const { user: LoggedInUser } = useContext(UserContext)
  const { data, isLoading, isSuccess } = useQuery<AxiosResponse<GetLeaveDto[]>, BackendError>(["articles", hidden], async () => new DropdownService().GetArticles(String(hidden)))
